...
export default function ActionPlanner({
  ...
  return (
    <div className="space-y-6" id="planner-main-container">
      ...
      <div className="bg-[#0A0A0A] border border-white/10 p-6 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        ...
        <div>
          ...
          <p className="text-xs text-white/50 mt-1 max-w-xl font-sans">
        ...
        <div className="flex gap-4 text-xs font-mono shrink-0">
          <div className="bg-white/5 border border-white/10 px-4 py-2.5 rounded-xl">
            <span className="text-white/40 block uppercase text-[10px] tracking-wider mb-0.5">Active Habits</span>
          ...
          <div className="bg-[#C4FF00]/10 border border-[#C4FF00]/20 px-4 py-2.5 rounded-xl">
            <span className="text-[#C4FF00] block uppercase text-[10px] tracking-wider mb-0.5">Current Rate</span>
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 border-b border-white/10 pb-4">
        ...
        <div className="flex flex-wrap gap-1.5 text-xs font-mono">
          {["all", "transport", "energy", "diet", "lifestyle"].map((tab) => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-lg border capitalize cursor-pointer transition-colors font-bold ${
                activeTab === tab 
                  ? "bg-[#C4FF00] border-transparent text-black" 
                  : "bg-[#0A0A0A] border-white/10 hover:border-white/30 text-white/60 hover:text-white"
              }`}
            >
              {tab}
            </button>
        ...
        <div className="flex items-center gap-2.5 text-xs font-mono">
          <span className="text-white/40 uppercase text-[10px] tracking-wider">Difficulty:</span>
          <div className="flex rounded-lg border border-white/10 overflow-hidden bg-[#0A0A0A]">
            {["all", "easy", "medium", "hard"].map((diff) => (
              <button
                key={diff}
                onClick={() => setDifficultyFilter(diff as any)}
                className={`px-3 py-1.5 border-r border-white/10 last:border-0 capitalize cursor-pointer transition-all ${
            ...
            return (
              ...
              >
                ...
                <div>
                  <div className="flex justify-between items-start">
                    <div className={`w-9 h-9 rounded-xl flex items-center justify-center shrink-0 border uppercase font-mono text-[9px] font-bold ${
                      action.category === "transport" ? "bg-amber-400/5 text-amber-400 border-amber-400/20" :
                      action.category === "energy" ? "bg-sky-400/5 text-sky-450 border-sky-400/20" :
                      action.category === "diet" ? "bg-rose-450/5 text-rose-400 border-rose-400/20" :
                      "bg-[#C4FF00]/5 text-[#C4FF00] border-[#C4FF00]/20"
                    }`}>
                      {getActionIcon(action.icon)}
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className={`text-[9px] uppercase font-mono font-bold px-2.5 py-0.5 rounded-md border ${
                        action.difficulty === "easy" ? "bg-emerald-450/5 text-emerald-400 border-emerald-450/20" :
                        action.difficulty === "medium" ? "bg-amber-450/5 text-amber-400 border-amber-450/20" :
                        "bg-rose-450/5 text-rose-400 border-rose-450/20"
                      }`}>
                        {action.difficulty}
                      </span>
                    </div>
                  ...
                  <p className="text-xs text-white/50 mt-1 leading-normal font-sans">
                    {action.description}
                  </p>
                </div>
                ...
                <div className="mt-6 pt-4 border-t border-white/5 flex items-center justify-between">
                  <div>
                    <span className="text-[9px] text-white/40 uppercase tracking-widest block font-mono">Offset value:</span>
                    ...
                    {isAdopted ? (
                      <>
                        <button
                          ...
                          className="px-3 py-1.5 bg-[#C4FF00] hover:bg-white text-black font-bold uppercase rounded-lg cursor-pointer transition-colors flex items-center gap-1 text-[10px]"
                        >
                          <Check className="w-3.5 h-3.5" /> Log Done
                        </button>
                        <button
                          onClick={() => onToggleAction(action.id, action.co2SavedPerWeek)}
                          className="px-2.5 py-1.5 text-rose-400 hover:text-rose-300 font-bold uppercase text-[10px] cursor-pointer"
                        >
                          Drop
                        </button>
                      </>
                      ...
                      <button
                        onClick={() => onToggleAction(action.id, action.co2SavedPerWeek)}
                        className="px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-bold uppercase rounded-lg text-[10px] cursor-pointer flex items-center gap-1.5 transition-all"
                      >
